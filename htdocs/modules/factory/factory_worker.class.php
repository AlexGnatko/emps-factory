...
//				$this->say($emps->db->sql_error());
    ...
    public function export_website($data, $ra){
        ...
        $this->echo_shell("tar pczf --overwrite {$export_path}/website.tar.gz -C {$htdocs}/../ .");
        $hostname = $cfg['hostname'];
        $uploads_dir = "/srv/upload/".$owner."/".$hostname;
        $this->echo_shell("tar pczf --overwrite {$export_path}/uploads.tar.gz -C {$uploads_dir}/../ .");
...
//	    echo $last_db_stats;
    ...
    public function start_service($row) {
        ...
        $out = null;
        $command = "cd {$row['path']} && setsid ".$row['command']." >/dev/null 2>&1 < /dev/null & echo $!";
